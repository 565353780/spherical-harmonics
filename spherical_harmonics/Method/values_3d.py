...
def get3DValue(degree, idx, phi, theta, method, method_name):
    ...
    match degree:
        ...
        case 1:
            return get3DBaseValue(idx, phi, theta, method) * getDeg1ThetaValue(
                idx, theta, method
            )
        case 2:
            return get3DBaseValue(idx, phi, theta, method) * getDeg2ThetaValue(
                idx, theta, method
            )
        case 3:
            return get3DBaseValue(idx, phi, theta, method) * getDeg3ThetaValue(
                idx, theta, method
            )
        case 4:
            return get3DBaseValue(idx, phi, theta, method) * getDeg4ThetaValue(
                idx, theta, method
            )
        case 5:
            return get3DBaseValue(idx, phi, theta, method) * getDeg5ThetaValue(
                idx, theta, method
            )
        case 6:
            return get3DBaseValue(idx, phi, theta, method) * getDeg6ThetaValue(
                idx, theta, method
            )
...
def getMathSH3DValue(degree, idx, phi, theta):
    ...
    for p, t in zip(phi, theta):
        value_list.append(getSH3DValueWithMethod(
            degree, idx, p, t, math, "math"))
    return value_list


def getNumpySH3DValue(degree, idx, phi, theta, dtype=numpy.float64):
    return toData(
        getSH3DValueWithMethod(degree, idx, phi, theta,
                               numpy, "numpy"), "numpy", dtype
    )
...
def getJittorSH3DValue(degree, idx, phi, theta, dtype=jittor.float64):
    return toData(
        getSH3DValueWithMethod(degree, idx, phi, theta, jittor, "jittor").to(
            phi.device
        ),
        "jittor",
        dtype,
    ...
    if method_name != "math" or not isinstance(phi, list):
        for degree in range(degree_max + 1):
            for idx in range(-degree, degree + 1, 1):
                values.append(getSH3DValue(
                    degree, idx, phi, theta, method_name, dtype))
        return values
        ...
        for idx in range(-degree, degree + 1, 1):
            for i in range(len(values_list)):
                values_list[i].append(
                    getSH3DValue(degree, idx, phi[i], theta[i], method_name)
                )
    return values_list
