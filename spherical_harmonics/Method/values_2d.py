...
def getNumpySH2DValue(degree, idx, phi, dtype=numpy.float64):
    return toData(getSH2DValueWithMethod(degree, idx, phi, numpy, 'numpy'), 'numpy', dtype)

def getTorchSH2DValue(degree, idx, phi, dtype=torch.float64):
    return toData(getSH2DValueWithMethod(degree, idx, phi, torch, 'torch'), 'torch', dtype).to(phi.device)

def getJittorSH2DValue(degree, idx, phi, dtype=jittor.float64):
    return toData(getSH2DValueWithMethod(degree, idx, phi, jittor, 'jittor').to(phi.device), 'jittor', dtype).to(phi.device)
